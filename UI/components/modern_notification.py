from PySide6.QtWidgets import QWidget, QLabel, QApplication, QGraphicsDropShadowEffect, QVBoxLayout, QPushButton, QHBoxLayout, QFrame, QGridLayout
from PySide6.QtCore import Qt, QTimer, QPoint, QPropertyAnimation, QEasingCurve, Signal, QObject, QSize, QDateTime, QPointF, QParallelAnimationGroup
from PySide6.QtGui import QPainter, QColor, QPixmap, QIcon, QFont, QPen, QPolygonF, QFontMetrics
...
class FloatingElement(QWidget):
    ...
    def _update_shape_morph(self):
        """更新形状渐变进度"""
        # 打印调试信息，确认定时器正在运行
        # print(f"Updating shape morph: {self.current_shape} -> {self.target_shape}, progress: {self.shape_morph_progress:.3f}")
        ...
        if self.shape_morph_progress >= 1.0:
            ...
            # print(f"Shape changed to: {self.current_shape}, new target: {self.target_shape}")
...
class ArtisticNotificationContent(QWidget):
    ...
    def _create_floating_elements(self):
        ...
        for i in range(num_elements):
            ...
            element.shape_type = shape_type  # 显式设置形状类型
    ...
    def sizeHint(self):
        ...
        # 计算总高度 (标题行 + 分隔线 + 消息文本 + 底部信息栏 + 边距)
        height = 24 + 1 + text_rect.height() + 40 + 40  # 标题高度 + 分隔线 + 文本高度 + 底部栏 + 上下边距
...
class NotificationManager(QObject):
    ...
    showNotificationSignal = Signal(str, str, int, object)  # message, level, duration, source
